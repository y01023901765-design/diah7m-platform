  ...
  return(<div style={{maxWidth:780,margin:"0 auto",padding:"20px 16px"}}>
    ...
    <div style={{marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
        ...
        <span style={{fontSize:15,padding:"2px 8px",borderRadius:6,background:`${LT.sat}15`,color:LT.sat,fontWeight:600}}>Phase 2</span>
      </div>
      <div style={{fontSize:16,color:LT.textMid}}>
    ...
    <div className="grid-3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:16}}>
      {[1,2,3].map(tier=>{
        ...
        return(<div key={tier} onClick={()=>setFilterTier(active?0:tier)}
          style={{background:active?`${lb.color}15`:LT.surface,borderRadius:LT.cardRadius,padding:14,
          border:`1px solid ${active?lb.color:LT.border}`,cursor:"pointer",transition:"all .2s"}}>
          <div style={{fontSize:22,fontWeight:900,color:lb.color,fontFamily:"monospace"}}>{cnt}</div>
          <div style={{fontSize:16,fontWeight:700,color:LT.text,marginTop:2}}>{L==='ko'?lb.ko:lb.en}</div>
        </div>);
      })}
    </div>
      ...
      <div style={{display:"flex",gap:4}}>
        {Object.entries(ARCHETYPE_LABELS).map(([k,v])=>{
          ...
          return(<button key={k} onClick={()=>setFilterArch(active?'':k)}
            style={{padding:"6px 10px",borderRadius:6,border:`1px solid ${active?LT.accent:LT.border}`,
            background:active?`${LT.accent}15`:LT.surface,color:active?LT.accent:LT.textDim,
            fontSize:15,fontWeight:600,cursor:"pointer"}}>{L==='ko'?v.ko:v.en}</button>);
        })}
      </div>
    </div>

    {/* Result Count */}
    <div style={{fontSize:16,color:LT.textDim,marginBottom:8}}>
    ...
    {filtered.slice(0,user?.plan==='PRO'||user?.plan==='ENTERPRISE'?100:10).map(s=>{
      ...
      return(
      <div key={s.id} style={{background:LT.surface,borderRadius:LT.smRadius,padding:"12px 14px",
        border:`1px solid ${LT.border}`,marginBottom:6,animation:"fadeIn 0.2s ease"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                ...
                <span style={{fontSize:15,padding:"1px 5px",borderRadius:4,background:`${tierLb.color}15`,color:tierLb.color,fontWeight:700}}>T{s.tier}</span>
              </div>
              <div style={{fontSize:15,color:LT.textDim}}>{s.sec} · {s.fac}{L==='ko'?'시설':' fac'} · {L==='ko'?ARCHETYPE_LABELS[s.a].ko:ARCHETYPE_LABELS[s.a].en}</div>
            </div>
          </div>
        </div>
        <div style={{display:"flex",gap:3,marginTop:6,flexWrap:"wrap"}}>
          {s.sat.map(st=>(<span key={st} style={{fontSize:15,padding:"2px 5px",borderRadius:3,background:`${LT.sat}12`,color:LT.sat,fontWeight:600}}>🛰️{st}</span>))}
        </div>
      </div>);
    })}
    ...
    {filtered.length>10 && user?.plan!=='PRO' && user?.plan!=='ENTERPRISE' && (
      <TierLock plan={user?.plan||'FREE'} req="PRO" lang={L}>
        <div style={{background:LT.surface,borderRadius:LT.cardRadius,padding:30,textAlign:"center"}}>
          <div style={{fontSize:16,fontWeight:800,color:LT.text,marginBottom:6}}>
            +{filtered.length-10} {L==='ko'?'종목 더보기':'more stocks'}
          </div>
          <div style={{fontSize:16,color:LT.textMid}}>PRO {L==='ko'?'플랜에서 전체 종목 감시':'plan for full stock monitoring'}</div>
        </div>
      </TierLock>
    )}
