...
export async function diagnose(gaugeData) {
  await loadSchema();
...
export function detectCrossSignals(systems) {
  const signals = [];
  const sm = {};
...
export function detectDualLock(systems) {
  const locks = [];
  const sm = {};
  systems.forEach(s => { sm[s.axis_id] = s; });
...
export function generateActionSignals(systems, crossSignals = [], dualLocks = []) {
  const signals = [];
