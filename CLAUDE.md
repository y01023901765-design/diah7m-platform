...
### 0단계: 게이지 관련 작업 시 추가 필수 확인
게이지·보고서·단위·코드 관련 작업이라면 **`memory/doctrine-gauge-audit.md`를 먼저 읽을 것.**
- 59게이지 코드 매핑 (pipeline ID → report code)
- 단위 포맷 기준 (fmtValue 기준표)
- 현재 충돌 해소 상태
이를 무시하고 작업하면 오늘 발생한 32개+ 단위 불일치가 재발한다.
