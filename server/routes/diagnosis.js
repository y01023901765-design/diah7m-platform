...
module.exports = function createDiagnosisRouter({ db, auth, engine, dataStore, state }) {
   ...
   * 경제 진단 → 한국어 서사 산문 변환 (엔티티 공용)
   * 공개 API (인증 불필요)
   *
   * Query params:
   *   entityType = country | province | district (default: country)
   *   entityName = "강남구" (default: 대한민국)
   *   entityCode = "KR-11680" (default: KR)
   *   parentName = "서울특별시" (optional)
   */
  router.get('/diagnosis/kr/narrative', async (req, res) => {
    try {
      const { buildNarrative } = require('../lib/narrative-engine');

      const entityType = req.query.entityType || 'country';
      const entityName = req.query.entityName || '대한민국';
      const entityCode = req.query.entityCode || 'KR';
      const parentName = req.query.parentName || null;
      ...
      if (engine && dataStore) {
        ...
        if (Object.keys(gaugeData).length > 0) {
          ...
          diag = await engine.diagnose(gaugeArr, {
            ...
            entityCode,
            parentName,
            productType: entityType === 'country' ? 'national' : entityType,
            frequency: 'daily',
          });
          isDemo = false;
        }
      ...
      res.json({ success: true, data: narrative, demo: isDemo });
    } catch (error) {
