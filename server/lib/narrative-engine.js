...
const METAPHOR = {
  ...
  O1: { // 수출(전년비%) → 근육 활력
    name: "수출(전년비)", bodyPart: "근육",
    "양호 ○": { metaphor: "근육 활력 양호", prefix: "수출이 증가하며 경제의 근육이 힘을 발휘하고 있다." },
    "주의 ●": { metaphor: "근육 활력 약화", prefix: "수출이 둔화되며 경제의 근육이 뻣뻣해지기 시작했다." },
    "경보 ★": { metaphor: "근육 활력 급락", prefix: "수출이 감소하며 경제의 근육이 크게 약해지고 있다." },
    diagnosisTemplate: {
      "양호 ○": (v,c) => `근육 활력 양호. 수출(${v}) 성장. 대외 수요 견조.`,
      "주의 ●": (v,c) => `근육 활력 약화. 수출(${v}) 둔화. 대외 수요 감소 신호.`,
      "경보 ★": (v,c) => `근육 활력 급락. 수출(${v}) 감소. 대외 수요 충격 진행 중.`,
    },
    narrativeTemplate: (v, c, grade) => {
      if (grade === "경보 ★") return `수출(O1)이 전년 동기 대비 ${v}로 감소했다. 경제의 근육이 힘을 잃고 있다. 수출은 한국 경제가 외부에서 산소를 끌어오는 통로다 — 통로가 좁아지면 내부 세포(가계)까지 타격이 미친다.`;
      if (grade === "주의 ●") return `수출(O1)이 전년 동기 대비 ${v}로 둔화됐다. 경제의 근육 활력이 약해지기 시작했다. 글로벌 수요 둔화의 초기 신호다.`;
      return `수출(O1)이 전년 동기 대비 ${v}로 성장세를 유지하고 있다. 경제의 근육이 정상적으로 작동하고 있다.`;
    },
  },
...
const STRUCTURAL_TEMPLATES = {
  3: { // 🔴위기
    headline: '대외수지·에너지 공급 기반이 위기 수준이다',
    body: '대외수지·에너지·환경 지표가 동시에 경보 상태다. 서비스수지 적자 확대, 에너지 생산 기반 약화, 환경 처리 과부하가 복합적으로 진행되고 있다. 이는 순환 건강(축1~7)의 단기 대응으로 해결되지 않는 구조적 위험이다. 재생 가능성 자체가 소멸되는 단계로, 외부 충격 시 급속 악화 위험이 매우 높다.',
  },
  2: { // 🟠경보
    headline: '대외수지·에너지 공급에 경보 신호가 감지된다',
    body: '주요 대외·에너지 지표에서 경보 수준의 악화가 확인된다. 현재 추세가 지속되면 중기적으로 구조적 위기로 전환될 가능성이 있다. 외화 유입 감소와 에너지 생산 기반 약화가 동시에 진행 중이다.',
  },
  1: { // 🟡주의
    headline: '대외수지·에너지 공급에 주의가 필요하다',
    body: '대외·에너지 관련 일부 지표에서 악화 추세가 관찰된다. 아직 위기 수준은 아니지만 지속 모니터링이 필요하다.',
  },
  0: { // 🟢양호/🔵관심
    headline: '대외수지·에너지 공급이 안정적이다',
    body: '주요 대외·에너지 지표가 안정적 범위 내에 있다.',
  },
};
    ...
    _v28_overallNarrative: (() => {
      ...
      if (_grades.structuralLevel) {
        ...
        if (_grades.structuralCode >= 3) {
          lines.push('  → 대외수지·에너지 공급 기반이 위기 수준. 순환 건강과 독립적인 재생 체력 평가.');
          lines.push('  → 구조 개혁 없이는 순환 건강이 양호해도 장기 지속 어려움.');
        } else if (_grades.structuralCode >= 2) lines.push('  → 대외수지·에너지에서 경보 수준의 변화 진행 중.');
        else if (_grades.structuralCode >= 1) lines.push('  → 대외수지·에너지에 주의 필요하지만 아직 안정적.');
        else lines.push('  → 대외수지·에너지 공급 양호.');
      }
      return lines.join('\n');
    })(),
